...
class PropertyValue extends Model
{
    ...
    const EVENT_GET_SLUG_VALUE = 'shopaholic.property.generate_value';
    ...
    public static function getSlugValue($sValue)
    {
        ...
        $sSlug = (string) $sSlug;
        $sEventValue = Event::fire(self::EVENT_GET_SLUG_VALUE, [$sValue, $sSlug], true);
        if ($sEventValue !== null) {
            $sSlug = $sEventValue;
        }
        ...
        while (!empty($obPropertyValue) && $obPropertyValue->value != $sValue) {
            $sTempSlug = $sSlug.'-'.$iCounter;
            $obPropertyValue = PropertyValue::getBySlug($sTempSlug)->first();
            $iCounter++;
        }
